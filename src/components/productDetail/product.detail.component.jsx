...
const ProductDetailComponent = ({ route, routePath }) => {
  ...
  const saveAndProceed = (imgUrl) => {
    const price = productDetails?.data
      ? Object.values(productDetails?.data?.price)[
          Object.keys(productDetails?.data?.price).indexOf(selectedQuantity)
        ]
      : 0;
    dispatch(
      setConfirm({
  ...
  useEffect(() => {
    if (productDetails?.data)
      setSelectedQuantity(Object.keys(productDetails?.data?.price)[0]);
            ...
            <div className="row">
              <div className="col-12 col-md-6 p-0 pb-3 px-md-3 productPictureAndCarausel">
                <img
                  ...
                  src={origin() + productDetails?.data?.imageUrls[0]}
                  alt="mainImg"
                />
              </div>
                ...
                {showBoard !== "none" && (
                  <div style={{ height: "300px", width: "600px" }}>
                    <div
                      ...
                      <DesignTool
                        board={showBoard}
                        saveAndProceed={(imgUrl) => saveAndProceed(imgUrl)}
                      />
                    </div>
                  </div>
                )}
